...
When calling `for row in search_twitter("cat pictures"):` the function only runs until the first data item is yielded, before printing - so we do not need to wait long for the first value. It will then continue until there is no more data to get.
...
![Incremental Loading](../../images/workshop_incremental_loading.png)
